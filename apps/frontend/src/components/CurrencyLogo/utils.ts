...
import { type Currency, NATIVE, type Token } from "@pancakeswap/sdk";
...
export const getTokenListTokenUrl = (
  token: Pick<Token, "chainId" | "address">,
) =>
...
export const getCurrencyLogoUrlsByInfo = memoize(
  (
    ...
    const logoUrl =
      chainId && address ? getTokenListTokenUrl({ chainId, address }) : null;
    return [
